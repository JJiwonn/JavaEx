...
/*
    함수형 프로그래밍
    : 데이터를 변경시키지 않고(불변), 작업 수행 규칭(함수)를 중심으로 프로그램을 작성하는 방식
    - 불변성, 함수의 일급 객체화, 부수 효과 최소화를 목표로 선언적 코드를 작성할 수 있음
    - 자바의 경우, 람다식 + 스트림을 활용, 함수형 프로그래밍을 작성할 수 있음
    
    람다식
    - 이름이 없는 즉석 함수(일회성으로 사용되는 함수)
    - 메서드를 간결하게 표현할 수 있으며, 코드를 읽기 쉽고 간단하게 작성 가능
    
    스트림
    - 데이터를 물결의 흐름으로 파악하고, 여러 함수의 로직의 조합을 거치면서 변경되고 변환되는 절차
...
//		testLambdaParams(10, 20, (x , y)-> x + y);
	...
	private static void testCallLambda() {
		...
		BiFunction<Integer, // 첫번째 입력 파라미터의 타입
		           ...
		           Integer> // 리턴 타입
		           findMax = (x , y) -> x > y ? x : y;
		 // 자바에서 미리 제공된 인터페이스를 사용할 때는 .apply 메서드로 호출할 수 있음
		System.out.println(findMax.apply(10,20));
		
	}
	
	private static void testCallLambda(int x, int y, MyFunction f) {
